...
const navigation = [
  ...
  { href: '/#panduan', label: 'Panduan kunjungan' },
  { href: '/#kunjungan', label: 'Lokasi & jam' },
] as const;
  ...
  return (
    <header className="border-b border-zinc-200 bg-white px-4 sm:px-6 lg:px-8">
      <div className="mx-auto flex h-18 max-w-350 items-center justify-between gap-5">
        ...
        >
          {navigation.map((item) => {
            const active = pathname === '/' && item.href === '/#beranda';
            return (
              <Link
                key={item.href}
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            className="interactive inline-flex h-10 items-center rounded-xl bg-[var(--home-gradient)] px-3 text-xs font-semibold text-white shadow-[0_10px_24px_#28665d24] hover:-translate-y-0.5 sm:px-4 sm:text-sm"
          >
            Cek antrean
          </Link>
        </div>
